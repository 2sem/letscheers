...
class GADInterstitialManager : NSObject, GADInterstitialDelegate{
    var window : UIWindow;
    var unitId : String;
    ...
    fileprivate static var _shared : GADInterstitialManager?;
    static var shared : GADInterstitialManager?{
        get{
            return _shared;
        }
    }
    ...
    init(_ window : UIWindow, unitId : String, interval : TimeInterval = 60.0 * 60.0 * 3.0) {
        ...
        super.init();
        //self.reset();
        if GADInterstitialManager._shared == nil{
            GADInterstitialManager._shared = self;
        }
    }
